...
public class RightScale_SecondOnScale extends AutoPath {
	 ...
	 */
	RightScale_CurveToScale driveToScale;
	...
	public RightScale_SecondOnScale() {
		...
		driveToScale = new RightScale_CurveToScale();
	...
	@Override
	public void execute() {
		if (null != turnToScale && turnToScale.isFinished() && null != elevatorToScale && elevatorToScale.isFinished()) {
			Robot.ahrs.reset();
			Robot.encLeft.reset();
			turnToScale.cancel();
			turnToScale = null;
